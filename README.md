...
과제 “트리플여행자 클럽 마일리지 서비스”과제를 위한 프로젝트는 5개 입니다.

비지니스서비스 3개, API Gateway서비스, Eureka서비스를 사용합니다.

비지니스 서비스

1. 이벤트 서비스 : 리뷰등을 등록하는것을 하나의 이벤트라고 가정합니다. 이벤트를 등록하기 위한 서비스입니다. 해당 서비스에서 요청타입(REVIEW,POST)별로 각각의 서비스를 호출합니다. 
2. 리뷰 서비스 : 리뷰를 위한 서비스 입니다. 발생한 이벤트에 따라 리뷰를 등록,수정,삭제를 할 수 있습니다.
3. 포인트 서비스 : 보상을 위한 서비스 입니다. 포인트를 부여 할 수 있고 조회 할 수 있습니다.

API GateWay서비스

1. 외부 또는 내부에서 해당 서버에 각 요청에따라 라우팅 서비스를 제공합니다. 옵션에 따라 Load balancing, Filter 등을 이용 할 수 있습니다.

Eureka 서비스

1. 각 서비스의 주소 및 포트 등을 동적으로 관리합니다.
